...
\subsection{Real-time Data Processing Requirements}
...
\midrule
RDP1
& ML model should classify the gesture based on training data used and label the gesture accordingly in real-time
& This ensures that the system can processes data and gives output in real-time, so that continuous translation can be achieved\\
\hline
RDP2
& ML model should classify the gesture and label it. Then the system should put that label through a text-to-speech system to say the word out loud
& This allows users to consume content more easily and without the need for visual aids, making written content more accessible to people with visual impairments, learning disabilities, or other conditions that make reading difficult.\\
\bottomrule
\end{tabularx}
