...
The approaches for the SRS verification plan can be peer reviews from other teams, reviews from our group and reviews from TAs. The requirements will be verified using the system tests that are listed in detail in the SystemTest Description section.


\subsection{Design Verification Plan}
Similar to the SRS verification plan, the approaches involve peer reviews from teammates and other teams and the reviews from TAs. Their feedback can be utilized to verify our design. In addition, asking poetential users of the device is another approache for design verification. To ensure that the actual device is implemented under the project’s goal and scope, we can ask people who regularly use ASL to test our device and ask them about the positives and negatives to get feedback.
...
Input: User starts the program
            
Output: The camera is opened and displays an image of the user
  
Test Case Derivation: The camera must be functional in order to detect hand motion.
            
How test will be performed: The user will try to start the program to see if the camera is able to capture video.

\item{TC-CFR2: Raspberry Pi Speaker\\}
  
Control: Manual, Static, Functional
            
Initial State: Raspberry Pi is placed with the camera and headphones/speaker
...
Input: User performs hand gestures (ex. hand gestures for a, b, or c)
...
Input: User performs hand gestures (ex. hand gesture for m, n, p)
...
\item{TC-MLFR4: Detecting multiple pairs of hands from multiple users\\}
...
Input: User will perform a gesture for a letter of the alphabet
...
Input: User will perform a gesture that corresponds to the letter of the alphabet
...
Input: A sequence of user’s hand gestures (ex. hand gestures for a, d, h, p, y)
